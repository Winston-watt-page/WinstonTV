...
export function Sidebar() {
  ...
  return (
    <nav
      aria-label={APP_NAME}
      className="hidden w-56 shrink-0 flex-col gap-1 border-r border-white/5 p-4 md:flex"
    >
      {NAV_ITEMS.map(({ view, icon: Icon, labelKey }, i) => (
        <button
          ...
          className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors focus:outline-none focus-visible:shadow-focus ${
            activeView === view
