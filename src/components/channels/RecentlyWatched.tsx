...
export function RecentlyWatched() {
  ...
  return (
    <section aria-label={t("recentlyWatched")}>
      <h2 className="mb-3 text-sm font-medium text-mist-300">{t("recentlyWatched")}</h2>
      <div className="flex gap-3 overflow-x-auto pb-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        {recent.map((channel) => (
          <button
            ...
            className={`flex shrink-0 flex-col items-center gap-1.5 rounded-lg p-2 focus:outline-none focus-visible:shadow-focus ${
              channel!.id === currentChannelId ? "bg-ink-700" : "hover:bg-ink-700/60"
            }`}
          >
            <ChannelLogo src={channel!.logo} name={channel!.name} size="md" />
            <span className="w-16 truncate text-center text-[11px] text-mist-400">
              {channel!.name}
            </span>
          </button>
        ))}
